...
class DecodeModel(object):
    def __init__(self, X_folds, y_folds, classifier, modelType = 'logistic', alpha_cluster = 0.05, alpha_FWE = 0.05, num_perm = 1000, **kwargs):
        '''
        ...
        alpha_cluster : float, optional
            cluster-defining threshold for cluster-based m.c. permutation test. The default is 0.05.
    ...
    def permutationTest_cluster(self):
        ...
        H0_upper_crit = self.criticalValue(H0_perm_acc, self.alpha_cluster)
...
class DecodeModel_temporal_gen(object):
    def __init__(self, X_folds, y_folds, classifier, alpha_cluster = 0.05, alpha_FWE = 0.05, num_perm = 1000, **kwargs):
        '''
        Creates a 'DecodeModel' class that implements permutation-based cross-validation.
    ...
    def permute(self, perm):
        ...
        if not self.generalize_condition:
            ...
            # Shuffle the target (y) vector
            target_shuffled = dcopy(self.y_folds_gen)
            [np.random.shuffle(target_shuffled[i,0]) for i in np.arange(self.nfolds)]  
            [np.random.shuffle(target_shuffled[i,1]) for i in np.arange(self.nfolds)]
            # Re-train with shuffled labels 
            self.CV_train(self.X_folds, target_shuffled)
            # Re-compute the CV accuracy  with the shuffled design
            H0_perm_acc, _  = self.CV_test(self.X_folds_gen, target_shuffled)                    
        return(H0_perm_acc)
